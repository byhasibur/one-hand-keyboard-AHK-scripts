...
*d::
{
    if GetKeyState("CapsLock", "P") {
        return  ; let ~CapsLock & d handle it instead
    }
...
~Space & z::
~Space & x::
~Space & c:: Send "{WheelUp 3}" ;scrollspeed:=5
~Space & v:: Send "{WheelDown 3}" ;scrollspeed:=5
~Space & b:: Send "#{PrintScreen}"
...
Space::
{
    ...
    isYouTube := (InStr(activeTitle, "YouTube") && (InStr(activeTitle, "Chrome") || InStr(activeTitle, "Brave") || InStr(activeTitle, "Comet")))
...
global LBUTTON_HELD := false ; Horizontal Scroll Toggle
...
RButton::
{
    ...
    if (timeSinceLast < 300 && timeSinceLast > 0) {
                ...
                gui50.Add("Button", "x2 y120 w50 h50 BackgroundTrans", "Win+Prt").OnEvent("Click", dothis30)
                ...
                gui50.Add("Button", "x62 y120 w50 h50 BackgroundTrans", "PrtScr").OnEvent("Click", dothis00)
    ...
    Sleep(100)
    copied := A_Clipboard
short := (StrLen(copied) > 30) ? SubStr(Trim(copied), 1, 30) "..." : Trim(copied)
    MouseGetPos(&tipX, &tipY)
    ToolTip("Copied: " short, tipX, tipY - 30, 5)
